...
> **Superseded in part by issue #454** — this record measures the tree as of
> `origin/main` @ `c357e13`, and the divider chain has since moved. Two of its
> conclusions need reading with that correction:
>
> 1. **The `divider_chain` figures below are historical.** The block is now
>    1317.66 × 70.29 µm = 92,618 µm², and the whole-chip sum 204,129 µm²
>    (0.2552 mm² after ×1.25, **1.70×**) rather than the 243,660 / 0.3046 /
>    2.03× recorded here. `layout/evidence/area-audit/area-audit.md` — the
>    machine-rendered report, regenerated by `python3 layout/run_pv.py area`
>    — always carries the current numbers; the tables *in this file* are a
>    frozen snapshot of what #442 measured, deliberately not rewritten.
> 2. **Lever 3's attribution below is wrong, though its measurement is
>    right.** "73.97 µm of no-diffusion band" is correct, but this record
>    treats the block's 73 Metal2 tracks as one population packable into
>    54.75 µm. They are three hierarchical bands, and packing only works
>    within one level. The dominant one was `div23_cell`'s own internal band
>    (46.50 µm of the 100.29, never packed), not the top-level band (16.50 µm,
>    already packed at #341). Packing the macro's band recovered 39,530 µm²,
>    65.9 % of this record's 60,006 µm² ceiling, by a different mechanism than
>    the ceiling assumed. See `layout/evidence/divider-chain-layout/
>    PROOF-macro-track-packing.md` and `PLL-FLOORPLAN.md` §5.8.
>
> **Lever 2's falsification is unaffected and still holds** — it rests on the
> reference netlist and on comp's share of the bbox, neither of which a
> routing change moves (comp's *share* rose 1.02 % → 1.46 % only because the
> bbox shrank).
