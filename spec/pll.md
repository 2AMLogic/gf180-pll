...
| 5 | [Period jitter](#period-jitter) | ≤ 1.0 % of the output period, RMS, **conditional on ≤ 20 mV pp `vdd_vco` ripple** | `all-slow`/−40 °C/2.97 V, band 5 (2.51 % RMS at 100 mV pp ripple, open-loop); closed-loop deterministic jitter measured at all 45 mandated PVT corners, 0.0508–0.2691 % RMS, PASS at every corner (`sim/period-jitter/`, #13) | **measured** (sensitivity, and the closed-loop **deterministic** half at 45/45); **derived** (the ripple condition); the **random (noise-driven) half is not measured and is not obtainable from any analysis this flow offers** — DR-023, owed at #520. The row is not discharged by the deterministic PASS. All three ingredients of the route that would close it are now built — the impulse-sensitivity-function extraction (`sim/period-jitter/isf-bringup/`, DR-030) and the per-device generator PSDs along the ring's own trajectory (`sim/period-jitter/sid-trajectory/`, DR-031, which also measures the stationary-approximation bound at −1.15 … +1.06 dB for the thermal generator at one corner) — but the **assembly** is not, and the flicker half needs an observation interval this row does not state, so nothing about this row's status moves |
...
**The `20·log₁₀(N)` transfer is now MEASURED at N = 6, not asserted from theory
(DR-027, #509).** Until this paragraph the multiplication above rested on
theory alone: no testbench in `sim/` had ever perturbed the reference edge **in
time** and read the consequence at the output. Every closed-loop record here
drives `REF` from a perfectly periodic pulse source, and
`sim/reference-input-contract` (#499, DR-019) — the closest existing campaign —
varies the reference *waveform shape* at a fixed phase, which is a different
question. `sim/reference-phase-transfer` closes that gap with a deterministic
bench needing no noise methodology: a single, known 1 ns reference phase step,
read out through the loop's own REF-vs-FB static phase error, **differentially
against a paired control run** that is the same deck with the step switched off
(the loop's own slow phase winding is common-mode and cancels — see the record
for why a single-run difference cannot separate the two).

Measured, at N = 6 / band 6 / f_ref = 25 MHz / f_out = 150 MHz, over the same
5-corner PVT subset `sim/reference-spur` uses
(`sim/reference-phase-transfer/records/20260925-080736-b722f33.md`, 5/5 PASS):

| Quantity | Measured | Against |
|---|---|---|
| In-band REF→output phase transfer | **15.529 … 16.366 dB** | `20·log₁₀(6)` = **15.563 dB** |
| Transfer error vs. the stated figure | **−0.035 … +0.803 dB** | — |
| Loop's tracking of the 1 ns step | 903 … 1004 ps tracked; **−4 … +97 ps untracked** | full tracking = 0 ps |
| Differential's own noise floor (`pair_resid`) | **−10 … +26 ps** | — |
| Baseline drift the pairing removed (`drift_ctl`) | **−1.410 … +0.085 ns** | up to 15× the residual |
| Transfer 120 ns after the step (**above** the loop bandwidth) | **−16.77 … −5.99 dB**, i.e. **21.6 … 32.7 dB below** the in-band figure | the roll-off this derivation also asserts |

So the multiplication this specification states holds on this design to within
about **1 dB** at N = 6, and the low-pass shape it assumes is measured at one
frequency above the loop bandwidth as well. "About 1 dB" rather than three
digits is deliberate: the untracked residual is only a few times the
differential's own solver-noise floor, which the record measures on every point
instead of assuming.

**This does not convert the exclusion into a number, and does not change what
is owed below.** A measured *gain* is not a reference-jitter *limit*: the limit
additionally needs a statistical reference phase-noise spectrum pushed through
this gain against an output jitter allocation, which needs the closed-loop noise
methodology DR-020 records as unavailable and #520 owns. What changes is that
the exclusion is now backed by a measurement rather than by theory — which is
the disposition DR-027 ratifies. Its limits are on the record's own face and in
DR-027 Decision 4: one N, five of 45 PVT points, two frequency points rather
than a swept curve, MOS-only, schematic-level, and **not** evidence about lock.
...
obligation twice; an empty owner column is the more useful fact. **This is a
different ask from the transfer figure measured above (DR-027).** Measuring that
a known phase step is multiplied by `20·log₁₀(N)` needs no noise process at
all — it is a transient, and it is done. Turning a *statistical*
reference-jitter spectrum into an output jitter allocation does, which is
exactly #520's still-missing methodology. **The transfer being measured does not
sequence, unblock, or otherwise change this paragraph's numeric-limit gap**; it
removes one of the two inputs that gap needed, and the other one is still
absent.
...
**One ingredient of the route that would close that gap is now built, and it
converges (DR-030).** DR-023 Decision 2 stated the condition that would overturn
its finding — "if `pss` resolves, or if an ISF pipeline demonstrates
convergence" — and half of it has now been tested rather than reasoned about.
`sim/period-jitter/isf-bringup/` measures the impulse sensitivity function of the
ring, `h(x) = Γ(x)/q_max`, by injecting a known charge at a known phase into a
copy of the whole VCO that shares one deck, and therefore one adaptive timestep
sequence, with an uninjected reference copy. The risk #520 named as load-bearing
— that Γ's accuracy would fail exactly at the switching edge, where the timestep
control is weakest — **is retired with a number**: halving the internal-timestep
ceiling **from 2 ps to 1 ps**, the last step of the sweep and the setting the
bring-up runs at, moves `h` by ≤ 0.02 % at every sampled phase, including the two
switching edges. (The qualifier is the claim: the *first* halving in that sweep,
8 ps → 4 ps, still moves `h` by up to 2.32 %, so 8 ps is not a converged setting
and "per halving" without naming the step would be false — see DR-030.) What the
bring-up *also* found is that the quantity the jitter
sum needs is not the one a reader would reconstruct from a per-node table: a
device's channel generator is a current source between drain and source, the two
nodes' ISFs are nearly equal on this ring, and the residue must be injected
directly rather than subtracted. **None of this produces a jitter number.**

**The other two ingredients now exist too, and so does the bound that #520's
option (B) was refused for want of (DR-031).** `sim/period-jitter/sid-trajectory/`
tabulates each ring device's channel-thermal and flicker generator at the bias it
actually occupies at 24 phases of the oscillation, together with the trajectory it
is evaluated along, at three PVT points of the mandated grid — sampled out of the
ISF bring-up's *own* reference deck, so the two ingredients share one phase axis
by construction (the two decks' periods agree to −3.7 × 10⁻⁶). Its load-bearing
result is the *cost of the stationary approximation*, and it is the weighted form
that matters: substituting a constant `S₀` into the jitter integral makes it wrong
by `S₀/S_eff` with `S_eff = Σh²S/Σh²`, and the ISF weighting moves that effective
density down 1.0–4.1 dB (thermal) and up 3.3–5.2 dB (flicker) from the plain cycle
average — so the unweighted span, 67–115 dB for a switching device, bounds nothing.
Against `S_eff`, biasing each device at its own peak-|I_d| phase reproduces the
channel-thermal generator to **−1.15 … +1.06 dB** for all four device classes,
i.e. 0.88–1.13× in jitter RMS.

**That still does not produce a jitter number, and DR-031 states why in its own
Decision 2**: the bound exists at the *one* PVT point where a committed `h` exists
while the unweighted spans move 40 dB across three corners; the weighted average
rests on an effective 2.4 of 12 sampled phases for the two device classes whose ISF
weight is exact; and the flicker term cannot be turned into an RMS at all until
this row states an observation interval (DR-031 Decision 5 — now listed in
[Verification owed](#verification-owed)). What remains unbuilt is the **assembly**:
`h_gen(x)` for all four device classes at the `S_id` grid's phase resolution, `h`
at the ISF's zero crossings extrapolated to `dq → 0`, the integral, and then the
45-point grid. DR-023's finding is narrowed by DR-030 and unchanged by DR-031: the
row is still owed at **#520**, and the table below is unchanged.
...
|---|---|---|
| [Period jitter](#period-jitter) | any **random** (noise-driven) jitter number at all — the closed-loop **deterministic** component (and the temperature/supply sweep it was taken over) now exists at all 45 mandated PVT corners, the full temperature × supply plane (`sim/period-jitter/`, 0.0508–0.2691 % RMS, PASS against the draft target at every corner); the random component remains unmeasured, and **not measurable on this toolchain** — not for want of device noise models, which `.noise` supplies per device and per mechanism, but because no periodic-steady-state/`pnoise` path exists on this repo's pinned ngspice-46 (`pss` is not a command in it, and `TRANNOISE` injects nothing) to weight those stationary PSDs over the ring's own switching trajectory, across which they move by 46.6 dB (thermal) and 95.9 dB (flicker). Re-derived rather than assumed: `sim/period-jitter/noise-toolchain-probe/`. **The 0.50 % RMS this row's [supply-ripple derivation](#period-jitter) leaves for the random/thermal contribution is an unverified budget, not a result** — see DR-023 §Decision 3. **What has changed since 2026-09-26 is the route, not the row**: the impulse-sensitivity-function pipeline #520 lists as alternative (A) now has **all three** of its ingredients built, and neither of the two records that built them moves this row. (i) Γ — `h(x) = Γ(x)/q_max` — is measured and converges (`sim/period-jitter/isf-bringup/`, DR-030): the risk #520 named as load-bearing, Γ's accuracy at the switching edge, is retired at ≤ 0.02 % over the last halving of the timestep ceiling (2 ps → 1 ps, the setting the bring-up runs at — *not* every halving: 8 ps → 4 ps still moves `h` by up to 2.32 %). (ii) The per-device generator PSDs *along the ring's own trajectory*, and the trajectory itself, are measured at three PVT points (`sim/period-jitter/sid-trajectory/`, DR-031), sampled out of the ISF bring-up's own reference deck so the two share one phase axis (periods agree to −3.7 × 10⁻⁶). DR-031 also measures the bound #520's option (B) was refused for want of, in the form the jitter integral needs — against the `h²`-weighted average `S_eff = Σh²S/Σh²`, biasing each device at its own peak-\|I_d\| phase reproduces the channel-thermal generator to **−1.15 … +1.06 dB** (0.88–1.13× in jitter RMS) for all four device classes, while the raw cyclostationary span that a reader would quote instead is 67–115 dB and bounds nothing. What remains is the **assembly** — `h_gen(x)` for all four device classes at the S_id grid's phase resolution, `h` at the ISF's zero crossings extrapolated to `dq → 0`, the integral, then the 45-point grid — plus three stated limits on the bound itself: it exists at one of 45 corners while the unweighted spans move 40 dB across three, the weighted average rests on an effective 2.4 of 12 sampled phases for the two device classes whose ISF weight is exact, and the flicker half needs the observation interval the row below now owes. So this row is owed exactly as before, by the same issue | #520 (`period-jitter`, random component); [DR-023](decision-records/DR-023-random-period-jitter-owner-and-cyclostationary-gap.md) re-points this row off the closed #13 (and its since-closed successor, #505); [DR-030](decision-records/DR-030-isf-gamma-converges-and-the-gap-moves-to-the-remaining-two-ingredients.md) records the Γ ingredient and narrows DR-023's falsification clause without overturning its finding; [DR-031](decision-records/DR-031-sid-along-the-trajectory-and-the-bound-option-b-owes.md) records the other two ingredients and the stationary-approximation bound, and overturns nothing |
| [Period jitter](#period-jitter) | **the observation interval (or measurement bandwidth) that makes "period jitter RMS" well defined for a `1/f^α` generator.** This row states ≤ 1.0 % of the output period, RMS, and states no interval. That is harmless for the deterministic component and for a white generator — the ISF integral `var(ΔT) = (1/ω₀²)·½·∫h²S_i dt` is derived for a delta-correlated source and gives a single-period variance — but the ring's flicker generator is *not* delta-correlated, and its contribution to a period-to-period RMS depends on the window over which that RMS is taken. The slope is now measured rather than assumed (`S ∝ f^−0.94997`, seven frequencies over 1 kHz–300 MHz, ≤ 2 × 10⁻⁴ dex of residual, `sim/period-jitter/sid-trajectory/`), so this is no longer a question about the device model: it is a missing definition in this row. Owed as a **specification** decision, not a measurement — choosing the interval changes what the target means, so no agent picks one, and DR-031 integrates the flicker term nowhere in the meantime | #520 (`period-jitter`, random component) — the same owner as the row above, because the assembly cannot report a flicker contribution until this is settled; raised by [DR-031](decision-records/DR-031-sid-along-the-trajectory-and-the-bound-option-b-owes.md) §Decision 5 |
...
| [Reference input](#reference-input) | **the `20·log₁₀(N)` in-band transfer figure — MEASURED at N = 6, with four named residuals** (DR-027). `sim/reference-phase-transfer/records/20260925-080736-b722f33.md` measures 15.529 … 16.366 dB against the stated 15.563 dB (error −0.035 … +0.803 dB), 5/5 PASS, and measures the roll-off above the loop bandwidth at one frequency (21.6 … 32.7 dB below the in-band figure). What remains owed is **not** the figure: (a) **other N** — this is evidence at N = 6 only, and extending it needs a `vco-tuning-range`-derived `vstart` table at another (N, band, f_out) triple; (b) **a swept transfer function** rather than two frequency points, which is #509's "option 1" and would let the loop bandwidth be read off this measurement instead of cited from `sim/loop-dynamics`; (c) **the remaining 40 PVT points**, on `sim/reference-spur`'s own justification for the same 5-corner subset; (d) **the passive process axes**, held at typical. Note also that this record is **not** evidence about lock — the design's own detector does not assert at four of its five corners at that release point, which the differential does not depend on (record Limitations (6); row 16 and #437 own that question) | **#509** (`reference-phase-transfer`) — figure **measured**; (a)–(d) unowned; DR-027 (the measurement, its limits, and its non-effect on the row below) |
| [Reference input](#reference-input) | a numeric reference-jitter limit to replace the current exclusion — which needs the closed-loop noise bench, i.e. this repository's noise methodology, **not** the closed-loop lock bench, and is a different ask from the transfer figure in the row above (DR-027): that row has now *measured* the gain a known step is multiplied by, and this row needs a statistical reference-jitter spectrum pushed through that gain into an output jitter allocation. One of the two inputs exists; the other does not. **It has no owner, and this row now says so rather than naming one.** Its prerequisite is the noise methodology tracked at #520 (the successor to issue #505, closed, which was itself the successor to issue #13, closed too — DR-023); the reference-jitter limit is a further measurement on top of that and is unowned today. The exclusion itself is defensible and is restated with its boundary in [Reference input](#reference-input); what is owed is the number, and its input-side half (`dtdv_worst`, the AM-to-PM coefficient at the worst legal reference slope) is a deliverable of the campaign two rows above | **unowned** — sequenced behind #520 |
