...
- **Methodology / criteria / limitations**:
  ...
  - **Supply step and ramp: the rates, stated.** One transient per corner
    ...
    contaminates the other. The **step** is 3.30 -> 3.63 V with a
    100n edge at t = 5.6u -- about 1/40 of the loop's own
    response time, i.e. a step as far as the loop is concerned. The **ramp**
    is 3.63 -> 2.97 V over 40 us,
  ...
  - **Waveform retained**: the step/ramp transient IS the argument for
    criterion 3, so it is kept -- decimated to 20e-9 s per sample as
    `corners/20260801-024935-c4fe724/supply_transient_<corner>.csv` (control node, LOCK flag,
...
- **Result**:
  ...
  **What this number is, and why it is small.** With the loop LOCKED, the
  ...
  - Settled control voltage over the whole grid: **1.466 .. 2.792 V**
    (lowest typical/-40C/2.97V, highest typical/27C/3.63V). Points outside DR-001 Decision
    2's usable 0.9-2.4 V window: **2** --
    typical/27C/3.30V typical/27C/3.63V. **FAIL**
  ...
  **THE SETTLING BUDGET, AND WHICH ROWS ARE ENTITLED TO BE READ AS
  ...
  9.3 us (9.6u is 1.03 tau), so a corner whose phase-acquisition transient
  ...
  **Consistency with #8's open-loop pushing.** The loop holds f_out fixed by
  ...
  between t = 16.0u and t = 22.4u (103.1 mV/us); settle.
...
- **Links**:
  ...
  - Retained waveforms: `corners/20260801-024935-c4fe724/supply_transient_*.csv`
