...
worst-case *guaranteed* floor: the lowest frequency each corner can reach,
...
2. **Closed-loop `period-jitter`'s random/noise-driven component has zero
   ...
   **0.50 % RMS** to this component, so the line is met in simulation for its
   ...
   #520 named, Γ's accuracy at the switching edge, at ≤ 0.024 % per halving of
   the internal-timestep ceiling — but a converging ingredient is not a
   converging pipeline, and the two ingredients that are missing are the ones
   that would do the cyclostationary weighting. So DR-023's clause is narrowed
   rather than fired, this row stays **UNMET**, and the unverified half of the
   jitter budget stays unverified. Until
