...
| 5 | [Period jitter](#period-jitter) | ≤ 1.0 % of the output period, RMS, **conditional on ≤ 20 mV pp `vdd_vco` ripple** | `all-slow`/−40 °C/2.97 V, band 5 (2.51 % RMS at 100 mV pp ripple, open-loop); closed-loop deterministic jitter measured at all 45 mandated PVT corners, 0.0508–0.2691 % RMS, PASS at every corner (`sim/period-jitter/`, #13) | **measured** (sensitivity, and the closed-loop **deterministic** half at 45/45); **derived** (the ripple condition); the **random (noise-driven) half is bounded, not measured: ≤ 0.338 % RMS at every one of the 45 mandated PVT corners** (upper bound, against the 0.50 % the ripple derivation leaves for it — `sim/period-jitter/random-bound/`, DR-032, which supersedes DR-020 Decision 1 and DR-023 Decision 2 for a bound). Scope: every generator in `vco` and the loop-filter resistor; the charge pump, PFD, divider and lock detector are argued small, not bounded (#580). No *estimate* of the random half exists — the ISF route's ingredients are built (DR-030, DR-031) but not assembled, and the bound does not need them |
...
methodology for a *reference* spectrum, which does not exist here: DR-020
records it as unavailable, and the random-jitter bound that closed #520
(DR-032) bounds the block's own generators rather than pushing a reference
spectrum through the loop. What changes is that
...
honest state is that the numeric limit is **unowned**, sequenced behind a
closed-loop reference-noise methodology that does not exist (DR-032's bound
covers the block's own generators only) — and [Verification owed](#verification-owed) now
...
a methodology this repository still lacks — #520 closed on a bound of the
block's own generators (DR-032), not on it. **The transfer being measured does not
...
**No *estimate* of the random (noise-driven) jitter exists, and none is
obtainable on this toolchain** (DR-023) — **but an upper bound now does, at
all 45 mandated corners, and it clears the budget (DR-032, below).** Every record's Methodology field discloses that
...
runnable. The measurement was owed at **#520**; the probes are committed at
...
45-point grid. DR-023's finding is narrowed by DR-030 and unchanged by DR-031;
after DR-031 the row was still owed at **#520**. (That assembly would give an
*estimate*; DR-032, next, answers the row's pass/fail without it.)

**The random half is now bounded, over the full mandated grid (DR-032).**
`sim/period-jitter/random-bound/` bounds it in three independent terms. **The
ring and the output buffer**: a `trnoise()` current source across every
channel, each sized at the **maximum of that device's own noise density over
its own trajectory** (`.noise` on each device standalone at its in-situ bias,
sampled down to the trajectory's own 2 ps resolution around each switching
device's peaks). Because the ISF weight `h²` is non-negative, a stationary
injection at a generator's maximum produces **at least** the period variance
the cyclostationary generator does, with the ring's true `h`: the stationary
approximation's error is one-signed by construction, which is the bound #520's
option (B) was refused for want of. The flicker half is folded into the same
injection through the closed loop's error transfer over every loop the as-built
filter admits at the 45° floor — so it needs **no observation interval** (the
bound is on the stationary variance, which no finite window's expected sample
variance exceeds). **The bias generator**, whose noise reaches the ring through
the slow shared nets VBP/VBN and so moves the frequency for many periods: a
small-signal `.noise` about its DC operating point, referred to the ring's
frequency through measured sensitivities and integrated through the same loop
envelope with its own flicker spectrum — checked against the transient, which
it over-predicts by ~9 %, the safe direction. It is the largest of the three
terms at every point. **The loop-filter resistor**: equipartition through the
control node. Result: **≤ 0.338 % RMS at every one of the 45 points**
(worst `sf_-40c_2.97v`; one-sided 95 % confidence), a 1.48× margin on
the 0.50 % allocation. It is an **upper bound, not an estimate**, and its
scope is stated: the charge pump, PFD, divider and lock detector are in-band
sources, argued small (the closed loop's ≤ 1.4 MHz low-pass, then the period
difference's `(2πfT)²` ≈ 3.4 × 10⁻³) but not bounded — owed at **#580**.

So the split the ripple derivation above makes is now evidenced on all three
lines:
...
| Random / noise-driven | **bounded**, 45/45 corners (DR-032; scope: every `vco` generator and the loop-filter resistor — in-band sources owed at #580) | ≤ 0.338 % RMS (upper bound) against the 0.50 % allocation |

A bound above 0.50 % RMS at any mandated corner would not have been a FAIL — a
bound can only clear a line, not miss it — but it would have left the
allocation unverified; DR-023 §Consequences named a *measured* random component
above 0.50 % as one of the conditions forcing a new record, and a re-run of
the bound after any VCO change is what keeps that condition checked.
...
|---|---|---|
| [Period jitter](#period-jitter) | a **bound on the random jitter of the in-band generators** — the charge pump, the PFD, the feedback divider and the lock detector. The random half of this row is otherwise **bounded at all 45 mandated PVT corners** — ≤ 0.338 % RMS, against the 0.50 % the [supply-ripple derivation](#period-jitter) leaves for it (`sim/period-jitter/random-bound/`, DR-032) — the ring's and output buffer's generators injected in a transient at the maximum of each one's own density over its own trajectory, the bias generator's bounded small-signal about its DC point, and the loop-filter resistor's through the control node. The four in-band blocks are outside that injection: they reach the output only through the closed loop's low-pass transfer (≤ 1.4 MHz over every admissible loop, ~1 % of `f₀`) and then the period difference's `(2πfT)²` weighting (≈ 3.4 × 10⁻³ at 1.4 MHz), which is an argument that they are small and not a bound. No *estimate* of the random half exists either; none is owed, since the bound answers this row's pass/fail. The observation-interval row [DR-031](decision-records/DR-031-sid-along-the-trajectory-and-the-bound-option-b-owes.md) §Decision 5 added here is retired by DR-032 Decision 3: the bound is on the stationary variance of the locked period sequence, so it holds for every interval, and the row still states none | **#580** (in-band generators). The VCO-side bound is DR-032's ([DR-032](decision-records/DR-032-random-period-jitter-bounded-over-the-grid.md), which supersedes [DR-020](decision-records/DR-020-random-period-jitter-is-not-obtainable-on-this-toolchain.md) Decision 1 and [DR-023](decision-records/DR-023-random-period-jitter-owner-and-cyclostationary-gap.md) Decision 2 for a bound); [DR-030](decision-records/DR-030-isf-gamma-converges-and-the-gap-moves-to-the-remaining-two-ingredients.md) and [DR-031](decision-records/DR-031-sid-along-the-trajectory-and-the-bound-option-b-owes.md) record the ISF route's ingredients, which an estimate would need |
