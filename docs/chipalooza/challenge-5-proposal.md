...
`sim/harness/corners.py` defines it. Four rows are not: **Output band**,
**Period jitter (open-loop sensitivity)**, **Power** and **Kvco** all come
from the `vco-tuning-range` campaign, which runs a **63-point PVT grid** —
seven bundles, adding `all-slow` and `all-fast`, the two combined bundles that
skew every device family together (passives included) rather than MOS alone.
That is a *superset* of the mandated grid, so those worst cases are taken over
a wider corner universe, and each of them binds at an `all-slow`/`all-fast`
corner the mandated grid does not contain. Until 2026-09-25 this table said
none of that: four rows quoted a corner a reader could not place, in a column
whose every other row meant "of 45". `sim/lib/check-pvt-coverage-claims.sh`
now fails this repository's CI if a row quotes a corner outside the mandated
grid without naming the grid it came from, if a row names a process bundle
`sim/harness/corners.py` does not define, or if a PVT corner count in a row is
neither the mandated grid size nor a count the cited record's committed
per-corner evidence produces. The mandated size is computed from the harness,
not written into the check, so widening the temperature or supply axis moves
what CI enforces in the same commit.
...
| Supply sensitivity — AC (ripple) budget | `vdd_vco` ripple ≤ 20 mV pp | Derived from the 2.51 %/100 mV pp measured sensitivity above | **derived, conditional** | Same record — `sim/vco-tuning-range/records/20260804-211600-f599a65.md` |
...
2. **Closed-loop `period-jitter`'s random/noise-driven component has zero
   ...
   **no point of this campaign varies the output band away from band 6 /
   ...
   tracked at issue #505 and the 200 MHz band-top campaign at #503. Until
