...
|---|---|---|
| Period jitter — closed-loop / random (noise-driven) jitter | `period-jitter` (deterministic component, 13 corners); `vco-tuning-range` (open-loop sensitivity substitute) | **partially** — `sim/period-jitter/` now has three records (deterministic component, 13 of 45 corners, 0.1111–0.2690 % RMS, PASS against the draft target at every point); random/noise-driven jitter is explicitly **not measured** by any of them (disclosed methodology gap, DR-002 Decision 5) and remains wholly owed |
| Period jitter — band sweep at non-nominal temp/supply | `period-jitter`, `vco-tuning-range` | **partially** — no longer wholly owed: `sim/period-jitter/records/20260906-024225-12bccda.md` closes the complete temperature × supply plane (−40/27/125 °C × 2.97/3.30/3.63 V) at nominal process, so both axes now have real closed-loop data (0.1111–0.2690 % RMS, worst cold-and-high-supply). Still owed: those same two axes crossed with the four skewed/extreme MOS bundles (32 of 45 points), and a sweep across the output **band**, which no record of this campaign varies — every point to date is at band 6 / 150 MHz. The cited open-loop ripple-jitter number is likewise one operating point, not a full grid |
