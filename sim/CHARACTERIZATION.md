...
| `period-jitter` | `sim/period-jitter/records/20260905-192724-a2ba48f.md` (`c51227a4fd78`) — first record, `typical`/27 °C/3.30 V only; `sim/period-jitter/records/20260906-015602-f9bef9d.md` (`184d49af1f7d`) — adds the four process-corner extremes at the same temperature/supply | Deterministic (control-ripple-driven) period/TIE jitter of the locked output, now measured at 5 of the mandated 45 corners (`typical`, `ff`, `ss`, `fs`, `sf`, all at 27 °C/3.30 V), same operating point as `reference-spur` (f_ref = 25 MHz, N = 6, f_out = 150 MHz, band 6, Icp code 0): **tj_rms 0.1333–0.2334 % of period** (worst: `typical`), well inside the 1.0 % RMS draft target at every measured point — **PASS against the draft, unratified threshold**. Lock and PFD DN-branch integration guards both PASS at every point (`ferr` up to 13.9 ppm, `dn_lvl` 0.162–0.213 against a 0.00089 floor, `dt_int_mean` 73.6–76.4 ps under the 100 ps ceiling). Random/noise-driven jitter is explicitly **not measured** by either record (methodology gap stated per DR-002 Decision 5 — TRANNOISE unsupported on this ngspice build for a free-running oscillator); the temperature (−40 °C, 125 °C) and supply (2.97 V, 3.63 V) axes remain entirely unswept — the full 45-point PVT grid for even the deterministic component remains owed. This sandbox's own compute-cost finding: a from-scratch full-45-point attempt with `-j8` parallelism caused every corner to exceed its 1200 s per-point ngspice timeout from CPU contention (0/45 completed) — serial (`-j1`) runs complete individually, at roughly 15–30 min/point depending on concurrent host load. | `spec/pll.md#period-jitter` (row 5, deterministic closed-loop half); #13 (part of #127) | **measured** (5 of 45 corners, deterministic component only); random component **not measured** — methodology gap disclosed |
...
|---|---|---|
| Period jitter — closed-loop / random (noise-driven) jitter | `period-jitter` (deterministic component, 5 corners); `vco-tuning-range` (open-loop sensitivity substitute) | **partially** — `sim/period-jitter/` now has two records (deterministic component, 5 of 45 corners, all at 27 °C/3.30 V, 0.1333–0.2334 % RMS, PASS against the draft target at every point); random/noise-driven jitter is explicitly **not measured** by either record (disclosed methodology gap, DR-002 Decision 5) and remains wholly owed |
| Period jitter — band sweep at non-nominal temp/supply | `period-jitter`, `vco-tuning-range` | **yes** — `period-jitter`'s records cover 27 °C/3.30 V only (5 of 45 corners: the process axis, not temperature/supply); the cited open-loop ripple-jitter number is likewise one operating point, not a full grid |
