...
- **Multi-record campaigns.** `cp-compliance` (DC + switching benches) and
  ...
  record. `lock-time` and `supply-sensitivity` each have a
  ...
  `output-range` was in that list until its full-grid record
  (`20260819-160843-4e32f91`) landed: its latest record is now the broad
  measurement, and the earlier waveform diagnostic is the one cited
  alongside it.
