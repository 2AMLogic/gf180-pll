# Record 20260731-194124-afa338c

- **Record ID**: 20260731-194124-afa338c
...
- **Netlist provenance**: schematic (`design/cp.sch` and the cells below it)
  ...
  `sim/cp-compliance/netlist-snapshots/20260731-194124-afa338c.spice`, SHA-256 `97856cc146493ffc31f96be947c1fc4fe55a302205f1e5749a6a1ffdb6a3cef4`.
...
- **Methodology / criteria / limitations**:
  ...
  - **Limitation (nominal skew only)**: `sw_stat_global = sw_stat_mismatch =
    ...
    centred on.
  - **Design revision under test**: this run measures the POST-#24 charge pump, in which the fixed dump-node diode clamp (`MCLPN1/2`, `MCLPP1/2`, which parked the node at 1.487 V) is replaced by `cp_dumpbuf` -- a complementary pair of unity-gain 5T OTAs holding the dump node at the control-node voltage (DR-005).  Stimulus, measurement and corner grid are UNCHANGED from record `20260731-122451-63e4b47`, which is the before-picture this is compared against; only the DUT moved, so the two records are comparable metric for metric.  One testbench change was needed and is deliberately a solver-EFFORT knob rather than a tolerance: `tb_cp_switch.sp` now sets `itl4 = 200` (transient Newton-iteration limit, default 10).  With the buffer, two MOS gates sit on the control node and the ideal source holding that node carries their displacement current, which numerically jitters at a few times 1e-14 A against the deck's 1e-14 A `abstol` even with the circuit in DC steady state; 10 iterations are not enough to settle it and ngspice collapses the timestep at 2 of the 135 switching points.  Every tolerance is unchanged and the converged solution is unchanged -- verified bit-for-bit at typical/27C/3.30V/Vctrl=1.65V, which converges either way.  This record does NOT supersede `20260731-122451-63e4b47`: that record is valid evidence for the design it measured.
...
- **Result**:
  ...
  **UP/DN mismatch (systematic, nominal skew)**:
  ...
  - Effective switching-time skew `w_up - w_dn`, worst over all corners and
    all three Vctrl points: -6.01046e-10 s
  - 50 % turn-on delay: UP -9.99945e-11 to -9.99642e-11 s, DN -3.12415e-11 to
    1.00297e-10 s; worst UP-vs-DN turn-on skew -2.00267e-10 s

  Full tables: `sim/cp-compliance/corners/20260731-194124-afa338c/cp_dc.csv` (per corner per
...
- **Links**:
  ...
  - Netlist snapshot: `sim/cp-compliance/netlist-snapshots/20260731-194124-afa338c.spice`
  - Raw logs: `sim/cp-compliance/corners/20260731-194124-afa338c/`
  - Extracted metrics: `sim/cp-compliance/corners/20260731-194124-afa338c/cp_dc.csv`,
    `cp_switch.csv`, `cp_curves.csv`
- **Timestamp / author**: 2026-07-31T19:41:58Z, agent-builder (issue #24)
