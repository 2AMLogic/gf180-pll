# Record 20260731-223426-640560e

- **Record ID**: 20260731-223426-640560e
...
- **Netlist provenance**: schematic (assembled by
  ...
  `sim/output-range/netlist-snapshots/20260731-223426-640560e.spice`, SHA-256 `ac58e450450cf88b893dace89b5a6dc200bbf7cd2924855e0beca7efe94c15b5`
...
- **Statistical convention**: N/A -- corner-matrix claim, not a distribution
  claim.
- **Result**:
| Edge | Target f_out | N | Status | vctrl_final | margin-to-0.9V | margin-to-2.7V |
|---|---|---|---|---|---|---|
| lo | 10 MHz | 8 | FAIL | 2.112 V | 1.21 V | 0.588 V |
| hi | 200 MHz | 32 | FAIL | 1.323 V | 0.423 V | 1.38 V |

  At least one edge did NOT reach a sustained-lock PASS inside the simulated window at this single corner -- see the per-row Status above and the transient-window discussion in Methodology (a FAIL/inconclusive row here means the window was too short to observe sustained lock, not that the loop cannot reach that edge; the un-fabricated final Vctrl reading is reported either way). **This does NOT bound the margin at the un-simulated
...
- **Links**:
  ...
  - Consumed design-input evidence (read-only, cited not re-derived):
    `sim/vco-tuning-range/corners/20260731-175947-0a12e6c/kvco_by_point.csv`
  - Netlist snapshot: `sim/output-range/netlist-snapshots/20260731-223426-640560e.spice`
  - Raw logs: `sim/output-range/corners/20260731-223426-640560e/`
- **Timestamp / author**: 2026-07-31T22:34:29Z, agent-builder (issue #12)
