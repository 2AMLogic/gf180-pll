#!/usr/bin/env bash
# gf180-pll :: reference-spur-band-top :: verify tb.json's configuration bits
...
# N -- and a spur read off a loop locked at the wrong N is not wrong-looking,
# it is just wrong: the carrier would sit on a different DFT bin and the
# reduction would report the ratio of whatever landed beside it.  A JSON
# manifest cannot call a bash function, so the codes are written into tb.json
# and this script asserts they are EXACTLY what the helper produces.
#
...
# sim/tests/test_reference_spur_band_top_grid.py runs it in CI.
...
echo "reference-spur-band-top config check -- N=${N}, Icp trim code=${TRIM}, band per point"
...
# read as though the band were static -- which is how sim/reference-spur's
# manifest correctly reads at 150 MHz and how this one must NOT.
...
cloop_check_instance_line "${HERE}/tb_reference_spur_band_top.sp" xdut
