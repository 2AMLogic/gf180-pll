# Record 20260731-195707-3e3814c

- **Record ID**: 20260731-195707-3e3814c
...
- **Netlist provenance**: schematic (`design/loop_filter.sch`, exported by
  ...
  `sim/loop-dynamics/netlist-snapshots/20260731-195707-3e3814c.spice`, SHA-256 `e4c2184f4be314f9dc5f9fef16d16a57ad4905a71b7f5fd287b8e1c1ee515b01`.
...
- **Environment provenance**:
  ...
  - Repo commit: `3e3814c` (DIRTY tree)
...
- **Result**:
  ...
  **Criteria applied** (stated up front, checked at every point below):
  ...
  per-event CHARGE asymmetry: |q_up + q_dn| = 41.9 fC median, 104 fC worst
  (`sim/cp-compliance`, worst at `fs/125C/2.97V Vctrl=2.4`).  In lock the loop nulls the average
  by standing off a static phase offset t_os = dQ/Icp = 19.5 ns, and the
  charge packet lands on C2 before redistributing into C1 through R.

  | Quantity | Worst-corner dQ | Median dQ |
  |---|---|---|
  | Vctrl ripple, peak (dQ/C2) | 51.55 mV | 20.76 mV |
  | Peak instantaneous df/f (kappa = 0.5/V) | 2.58 % | 1.04 % |
  | Bound on the TIE it can accumulate | 472.6 ps | 79.56 ps |
  ...
  model of the measured filter; the as-built 2.02 pF is the knee.
...
- **Links**:
  ...
  - Consumed design-input evidence (read-only): `sim/vco-tuning-range/corners/20260731-081628-239e73b/kvco_by_point.csv`,
    `sim/cp-compliance/corners/20260731-122451-63e4b47/cp_dc.csv`, `sim/cp-compliance/corners/20260731-122451-63e4b47/cp_switch.csv`
  - Netlist snapshot: `sim/loop-dynamics/netlist-snapshots/20260731-195707-3e3814c.spice`
  - Raw logs: `sim/loop-dynamics/corners/20260731-195707-3e3814c/` (81 filter-impedance +
    18 loop-AC cross-check logs)
  - Extracted metrics: `sim/loop-dynamics/corners/20260731-195707-3e3814c/filter_rc.csv`
...
- **Timestamp / author**: 2026-07-31T19:58:07Z, agent-builder (issue #10)
