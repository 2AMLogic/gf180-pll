...
def rect_frame(
    ...
    ``guard_ring()`` uses for its tap strips -- rather than as a filled
    ``rect()``. The distinction is load-bearing for ``nwell``: a filled
    ...
    surfaces at LVS, as an unrecognisable device. This helper exists so the
    hollow shape is a single named primitive with its own regression tests
    rather than an easily-mistyped four-`rect()` idiom at each call site.
