...
#
# NOTE: `gh pr view --json` has no `merged` field on gh >= 2.98.0 ("Unknown
# JSON field: merged") — the field was renamed/replaced by `mergedAt`, a
# timestamp string when merged and `null` otherwise (#234). Use `mergedAt` and
# derive the boolean from null-vs-non-null below.
...
# `.mergedAt` is a timestamp string when merged, `null` (or absent) when not —
# there is no boolean field to read here (see the #234 note above). Treat any
# non-null value as merged; `// null` supplies the default when the field is
# absent entirely (the pre-#6781 forge-shim shape below still needs this).
