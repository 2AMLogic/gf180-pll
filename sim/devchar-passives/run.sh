...
{
  simenv_provenance "devchar-passives (capacitors)" \
    "sim/devchar-passives/tb_cap_cv.sp" "${CAP_CORNERS}"
  cat <<'EOF'
...
} >"${HERE}/results/cap_summary.csv"

{
  simenv_provenance "devchar-passives (capacitor C-V curves)" \
    "sim/devchar-passives/tb_cap_cv.sp" "${CAP_CORNERS}"
  echo "${CAP_CURVE_HEADER}"
  cat "${WORK}"/cap_*.cur | sort -t, -k1,1 -k2,2 -k3,3n -k4,4n
} >"${HERE}/results/cap_cv.csv"

{
  simenv_provenance "devchar-passives (resistors)" \
    "sim/devchar-passives/tb_res.sp" "${RES_CORNERS}"
  cat <<'EOF'
...
{
  simenv_provenance "devchar-passives (resistor temperature coefficient)" \
    "sim/devchar-passives/tb_res.sp" "${RES_CORNERS}"
  cat <<'EOF'
...
} >"${HERE}/results/res_tempco.csv"

echo "devchar-passives: wrote results/cap_summary.csv, cap_cv.csv, res_summary.csv, res_tempco.csv"
