...
| `period-jitter` | `sim/period-jitter/records/20260905-192724-a2ba48f.md` (`c51227a4fd78`) — first record, `typical`/27 °C/3.30 V only; `sim/period-jitter/records/20260906-015602-f9bef9d.md` (`184d49af1f7d`) — adds the four process-corner extremes at the same temperature/supply; `sim/period-jitter/records/20260906-024225-12bccda.md` (`0a177fb30317`) — adds the remaining eight `typical` points, completing the full 3 × 3 temperature × supply plane at nominal process (the first data this campaign has on either of those two axes) | Deterministic (control-ripple-driven) period/TIE jitter of the locked output, now measured at **13 of the mandated 45 corners**: the process axis at 27 °C/3.30 V (`typical`, `ff`, `ss`, `fs`, `sf`) plus the complete temperature × supply plane at `typical` (−40/27/125 °C × 2.97/3.30/3.63 V). Same operating point throughout as `reference-spur` (f_ref = 25 MHz, N = 6, f_out = 150 MHz, band 6, Icp code 0). **tj_rms 0.1111–0.2691 % of period across all 13** — worst `typical`/−40 °C/3.63 V, best `typical`/125 °C/2.97 V; the temperature/supply plane alone spans 0.1111–0.2691 %, i.e. the two new axes move the number by about 2.4×, more than the process axis did (0.1333–0.2334 %), and the trend is monotone in the sensible direction (colder and higher-supply is worse — a faster ring, so the same control-ripple voltage buys more period deviation). Every point is **well inside the 1.0 % RMS draft target — PASS against the draft, unratified threshold, at all 13**. Lock and PFD DN-branch integration guards PASS at every point (worst |`ferr`| 29.8 ppm, `dn_lvl` ≥ 0.148 against a 0.00089 floor, `dt_int_mean` ≤ 80.9 ps under the 100 ps ceiling). Random/noise-driven jitter is explicitly **not measured** by any of the three records (methodology gap stated per DR-002 Decision 5 — TRANNOISE unsupported on this ngspice build for a free-running oscillator). The 32 still-unmeasured points are the four skewed/extreme MOS bundles off-nominal in temperature or supply; the manifest declares all 45, so what is owed there is compute, not design. Compute-cost note, corrected: the earlier records' "a `-j8` full-grid attempt timed out all 45 points" finding was **self-oversubscription, not an inherent per-point cost** — `sim/run_corners.py`'s `-j` is process-level fan-out while this host's ngspice spawns 8 OpenMP threads per process, so `-j8` demanded 64 threads of 8 cores. `sim/harness/omp.py` now budgets those threads; this record's own eight points completed 8/8 in 4015 s wall (`-j4`, 2 threads per worker) against ~200 competing ngspice threads from unrelated concurrent tenants on the same host. | `spec/pll.md#period-jitter` (row 5, deterministic closed-loop half); #13 (part of #127) | **measured** (13 of 45 corners, deterministic component only); random component **not measured** — methodology gap disclosed |
