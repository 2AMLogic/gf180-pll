...
| Output duty cycle — no measurement exists | `output-range` (adjacent scope, not this measurement) | **yes** — no campaign directory reports a duty-cycle number |
| Output levels and drive — loaded-output swing/edge-rate | (none) | **yes** — every recorded frequency measurement is against an essentially unloaded buffer |
