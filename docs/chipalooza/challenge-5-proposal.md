...
| Period jitter, **closed-loop, random/noise-driven** | Same 1.0 % RMS line | **Zero records.** None of the five deterministic-component records above measures this — a disclosed methodology gap (DR-002 Decision 5; ngspice `TRANNOISE` produces no injected noise on this repo's pinned build, and turning its `trnoise()` PWL source into a credible device-noise-equivalent figure needs a noise-PSD calibration this repository has not done). Tracked at issue #520 — the measurement itself. Two closed issues sit behind it, and the chain is stated rather than hidden: #13, which this row named until 2026-09-25, closed 2026-09-08 and left the gap with no owner at all for seventeen days; #505 was filed to repair that, and closed `completed` on 2026-09-25 once DR-020 landed, because its scope was the *ownership and disposition* of the gap and not the number. #520 is the successor DR-020's own filing names as the owner of the number. **DR-020 records the disposition**: the component is not obtainable from any analysis this toolchain offers (no transient device-noise model in ngspice at all; `.noise` needs a DC operating point an oscillator does not have; `trnoise()` injects a chosen amplitude), so what is owed is a methodology before a number — a validated noise-referral pipeline, a simulator that has the analysis natively, or a silicon measurement. The ≤ 1.0 % RMS target is **not** relaxed by that record | **UNMET — explicitly, not omitted.** This is the one row this proposal cannot report a number for at any maturity | issue #520 (open) — the measurement; issue #505 (closed) — the bookkeeping predecessor that filed it; DR-020, which re-points `spec/pll.md`'s Verification-owed row for this gap off the closed #13 and states why no number can be produced pre-silicon |
| Phase noise | not spec'd (DR-002 Decision 5) | n/a by design | **N/A, by design** | `spec/pll.md#phase-noise` |
| Reference spur | ≤ −55 dBc | −57.0…−72.7 dBc measured at 150 MHz (5 spanning corners); scaled to the binding 200 MHz, the two coldest corners land at −54.5/−54.9 dBc (0.1–0.5 dB over the line). **That measurement runs with charge-pump mismatch off.** The derivation it cross-checks has since been re-priced by DR-018 (#483): with the UP/DN current mismatch ("term 1") counted at its statistical 3σ instead of its systematic value, and the statistical residual refreshed to the corner-combined Monte Carlo campaign, the deliberately conservative 200 MHz stack lands at ≈ −57.0 dBc with term 1 at its measured 17.48 % and **−56.6 dBc** at its ±20 % budget — **1.6 dB inside the line**, where the older −61 dBc derivation, which left term 1 out, implied a ~6 dB reserve | **PASS at 150 MHz (5/5 corners); UNMET at the scaled 200 MHz binding point for 2/5 corners** — 5 of 45 PVT points measured, not the full grid. The derived stack including mismatch clears the line, but by 1.6 dB rather than ~6 dB (DR-018), so the reserve this row was once argued on is mostly spoken for. The owed work — a direct 200 MHz measurement and the other 40 PVT points — was handed by `spec/pll.md` to #145, which is closed; it is filed as **#510** | `sim/reference-spur/records/20260816-132150-5f405e7.md`; `spec/pll.md#reference-spur` (the re-priced derivation table); `spec/decision-records/DR-018-cp-term1-mismatch-budget-derived-from-the-spur-line.md`; issue #510 (open) |
...
| Lock time (small-signal settling) | < 100 µs to the ratified Lock criterion (output frequency within ±0.1 % **and** static phase error at the PFD inputs ≤ 1 ns) | Worst 71 µs (`f_ref` = 1 MHz, 4 legs); structural floor ≈ 43 µs (set by `1/(2πRC1)`, a derivation rather than a grid minimum); 120/140 cells meet the line, 0/140 meet the dropped < 20 µs stretch. The settling half of this row shares the Loop bandwidth row's 81-point filter-impedance basis, not the MOS grid (§5.0); the static-phase half below is measured on the mandated 45. **But the criterion this time is measured *to* is not reached at 2 of the 45 mandated corners** (DR-012): closed-loop, the loop settles at **1.227 ns** of static phase at `ff`/27 °C/3.63 V and 1.049 ns at `typical`/−40 °C/3.63 V against the ratified ≤ 1 ns — nominal skew, systematic only, before the 0.576 ns statistical mismatch term — so at those corners there is no instant for a lock time to be measured to. The axis it is lost on is the control voltage, not the corner: at `VCTRL` = 0.90 V the charge pump's residual charge alone exceeds the whole criterion at 36 of 45 corners, open-loop | **MET as a settling estimate (not cold-start) — and NOT MET at 2 of 45 corners**, where the Lock criterion itself is never reached, exactly as `spec/pll.md`'s own row states it. DR-012 Decision 1 keeps the criterion unrelaxed, so this is a design gap, not a spec one. Its settled value at 15 further over-bound corners is owed at #399 (open); the design resolution, which DR-012 deliberately does not pick, had no issue and is filed as **#511** | `sim/loop-dynamics/records/20260731-202550-82af5a9.md` (settling); `sim/supply-sensitivity/records/20260916-051708-8cedbba.md` (the 1.227 / 1.049 ns settled static phase); `spec/decision-records/DR-012-static-phase-offset-evidence.md`; issue #399 (open); issue #511 (open) |
...
2. **Closed-loop `period-jitter`'s random/noise-driven component has zero
   ...
   **DR-020 records the disposition of the first**: the random component is
   ...
   `completed` the same day DR-020 landed; #520 owns the measurement. A defect in the
...
3. **Closed-loop `lock-time` and `output-range` full-PVT grids exist but do
   ...
   `supply-sensitivity`'s findings were (its record names #9/#10/#11 for
   each class of finding — though all three of those issues have since
   closed, which is item 4 below); closing that gap is separate follow-on
   work, not covered by this proposal.
4. **`supply-sensitivity`'s three FAILing criteria are disclosed design
   ...
   corner). **They have no open owner.** The record routes each class of
   finding by name to #9 (`pfd_cp`, post-#24), #10 (`loop-dynamics`) and #11
   (`lock_detector`) — and all three of those issues are now closed, so what
   the record captured as a routing is today a recorded finding with nobody
   holding it. Until 2026-09-25 this item said instead that they "are
   already routed to their owning issues", which was true when written and
   false by the time anyone read it. Filed as **#506**; no recorded verdict
   or value changes, and `sim/` is append-only, so the record's own routing
   text stands as the history it is.
