...
`cp_array`'s composed cell used to inherit its sub-cells' own standalone pin
...
layers; only the 34/10 label purpose differs. The committed `cp_array.gds` was
...
| Check | Expected | Got | Verdict |
|---|---|---|---|
| `cp_array` DRC, table `main` | clean | `DRC clean: cp_array (D), 0 violations` | **PASS** |

`drc-clean/cp_array.drc.stdout.log` and `drc-clean/cp_array_main.lyrdb` are that run's own
