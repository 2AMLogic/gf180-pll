...
| `period-jitter` | `sim/period-jitter/records/20260905-192724-a2ba48f.md` (`c51227a4fd78`) — first record, `typical`/27 °C/3.30 V only; `sim/period-jitter/records/20260906-015602-f9bef9d.md` (`184d49af1f7d`) — adds the four process-corner extremes at the same temperature/supply; `sim/period-jitter/records/20260906-024225-12bccda.md` (`0a177fb30317`) — adds the remaining eight `typical` points, completing the full 3 × 3 temperature × supply plane at nominal process (the first data this campaign has on either of those two axes); `sim/period-jitter/records/20260906-063728-f3c9c23.md` (`ea55286e1112`) — adds the `ff` and `ss` bundles' complete temperature × supply planes (18 points), the first record of this campaign taken against a **clean tree**, and the first with an overall status of **FAIL** | Deterministic (control-ripple-driven) period/TIE jitter of the locked output, now measured at **29 of the mandated 45 corners**: the process axis at 27 °C/3.30 V (`typical`, `ff`, `ss`, `fs`, `sf`), plus the complete temperature × supply plane (−40/27/125 °C × 2.97/3.30/3.63 V) at each of `typical`, `ff` and `ss`. Same operating point throughout as `reference-spur` (f_ref = 25 MHz, N = 6, f_out = 150 MHz, band 6, Icp code 0). **tj_rms 0.0508–0.2691 % of period across all 29** — worst `typical`/−40 °C/3.63 V (unchanged), best now `ss`/125 °C/3.30 V, which widens the measured span from 2.4× to 5.3× and improves the worst-case margin claim's basis rather than eroding it: every one of the 29 is **inside the 1.0 % RMS draft target — PASS against the draft, unratified threshold, at all 29**, worst-case margin 3.7×. The two new bundles also correct a trend the nominal-process plane suggested: **"colder and higher-supply is worse" is not process-independent.** Temperature is monotone at every bundle (hotter is better), but the supply trend's *sign* flips with process — at `ff` jitter falls as supply rises (−40 °C: 0.2484 → 0.2477 → 0.2173 %; 27 °C: 0.2351 → 0.2149 → 0.1994 %), while at `ss` it rises (−40 °C: 0.1420 → 0.1703 → 0.2240 %; 27 °C: 0.0537 → 0.1457 → 0.1732 %). **The FAIL is a guard defect, not a design finding, and is filed as #273**: 2 of the 18 new points (`ff`/27 °C/2.97 V and `ff`/27 °C/3.63 V) fail the manifest's own `ferr` lock gate at 4.00e-2 against a ±1e-3 ceiling, but both loops are locked — `fout` 150.006/149.976 MHz (40/160 ppm of target), `ffb` within 110 ppm of 25 MHz, measured N = 6.000, and jitter inside the range their seven `ff` neighbours produce. The gate's `phi_a`/`phi_b` samples are `trig`/`targ` edge-pair delays, so each is wrapped into one reference period; at those two points `phi_a` ≈ 39.81/39.86 ns (one 25 MHz period minus a sub-200 ps phase error) against `phi_b` ≈ −0.19/−0.13 ns, making the apparent drift exactly one reference period per microsecond — i.e. 0.04. The value reproduced to seven digits in a separately-launched run of the same grid, so it is deterministic, not noise. The record is **not** edited to say otherwise; a wrap-safe gate and a superseding re-run of those two points are #273's job. Cross-host reproducibility, which this record was deliberately overlapped to test: `ff` and `ss` at 27 °C/3.30 V were previously measured with ngspice-46 on Linux/x86-64 and re-measured here with ngspice-46 on macOS/arm64 — 0.2072 → 0.2149 % (+3.7 % relative) and 0.1333 → 0.1457 % (+9.3 %), same order and same verdict, not bit-identical. Other guards PASS at every one of the 18 (worst passing |`ferr`| 30.8 ppm, `dn_lvl` ≥ 0.124 against a 0.00089 floor, `dt_int_mean` ≤ 85.3 ps under the 100 ps ceiling). Random/noise-driven jitter is explicitly **not measured** by any of the four records (methodology gap stated per DR-002 Decision 5 — TRANNOISE unsupported on this ngspice build for a free-running oscillator). The 16 still-unmeasured points are the `fs` and `sf` bundles off-nominal in temperature or supply; the manifest declares all 45, so what is owed there is compute, not design. Compute-cost note, corrected: the earlier records' "a `-j8` full-grid attempt timed out all 45 points" finding was **self-oversubscription, not an inherent per-point cost** — `sim/run_corners.py`'s `-j` is process-level fan-out while this host's ngspice spawns 8 OpenMP threads per process, so `-j8` demanded 64 threads of 8 cores. `sim/harness/omp.py` now budgets those threads; this record's own eight points completed 8/8 in 4015 s wall (`-j4`, 2 threads per worker) against ~200 competing ngspice threads from unrelated concurrent tenants on the same host. | `spec/pll.md#period-jitter` (row 5, deterministic closed-loop half); #13 (part of #127); #273 (the `ferr` gate defect) | **measured** (29 of 45 corners, deterministic component only), PASS against the draft target at all 29; latest record's overall status is **FAIL** on a non-wrap-safe lock gate at 2 locked corners (#273), not on the jitter claim; random component **not measured** — methodology gap disclosed |
