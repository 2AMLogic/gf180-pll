...
| `period-jitter` | `sim/period-jitter/records/20260905-192724-a2ba48f.md` (`c51227a4fd78`) | **First record for this campaign** (zero records before this one). Deterministic (control-ripple-driven) period/TIE jitter of the locked output, one nominal corner only (`typical`/27 °C/3.30 V), same operating point as `reference-spur` (f_ref = 25 MHz, N = 6, f_out = 150 MHz, band 6, Icp code 0): **tj_rms 0.2334 % of period** (314 cycles measured), well inside the 1.0 % RMS draft target — **PASS against the draft, unratified threshold**. Lock and PFD DN-branch integration guards both PASS at this point (`ferr` 1.62 ppm, `dn_lvl` 0.175 against a 0.00089 floor, `dt_int_mean` 73.7 ps under the 100 ps ceiling). Random/noise-driven jitter is explicitly **not measured** by this record (methodology gap stated per DR-002 Decision 5 — TRANNOISE unsupported on this ngspice build for a free-running oscillator); the full 45-point PVT grid for even the deterministic component remains owed. | `spec/pll.md#period-jitter` (row 5, deterministic closed-loop half); #13 (part of #127) | **measured** (1 of 45 corners, deterministic component only); random component **not measured** — methodology gap disclosed |
...
|---|---|---|
| Period jitter — closed-loop / random (noise-driven) jitter | `period-jitter` (deterministic component, one corner); `vco-tuning-range` (open-loop sensitivity substitute) | **partially** — `sim/period-jitter/` now has one record (deterministic component, `typical`/27 °C/3.30 V only, 0.2334 % RMS, PASS against the draft target); random/noise-driven jitter is explicitly **not measured** by that record (disclosed methodology gap, DR-002 Decision 5) and remains wholly owed |
| Period jitter — band sweep at non-nominal temp/supply | `period-jitter`, `vco-tuning-range` | **yes** — `period-jitter`'s new record is one nominal corner only; the cited open-loop ripple-jitter number is likewise one operating point, not a full grid |
