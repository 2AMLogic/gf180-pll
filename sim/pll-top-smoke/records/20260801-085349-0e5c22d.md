# Record 20260801-085349-0e5c22d

- **Record ID**: 20260801-085349-0e5c22d
...
- **Netlist provenance**: schematic (`design/pll_top.sch`, exported by
  ...
  `sim/pll-top-smoke/netlist-snapshots/20260801-085349-0e5c22d.spice` (exported subcircuit +
...
- **Environment provenance**:
  ...
  - Repo commit: `0e5c22d` (DIRTY tree)
...
- **Methodology / criteria / limitations**:
  ...
  - **Lock criterion**, stated before the run and applied to it:
    1. **Frequency** -- the residual fractional frequency error, measured as
       the DRIFT of the REF->FB phase between t = 14.0u and t = 17.0u,
    ...
    2. **Phase** -- the static REF->FB phase error at t = 17.0u must be
  ...
  - **Simulator settings**: `.tran 20n 20u 0 100p`,
  ...
  - **Waveform retained**: the acquisition transient is the argument here, so
    it is kept -- decimated to 20e-9 s per sample (999 rows) as
    `corners/20260801-085349-0e5c22d/lock_transient_typical_27c_3.30v.csv`, never as a rawfile. The deck
...
- **Result**:
  ...
  |---|---|---|---|---|
  | 1 | residual frequency error (phase drift 14.0u -> 17.0u) | <= 1e-3 | 1.83983e-03 | **FAIL** |
  | 2 | static phase error, REF -> FB, at 17.0u | <= 0.02 of a reference period | 14.05 ns | **FAIL** |
  | 3 | divide ratio f_out / f_fb | 8 +/- 0.01 | 7.99988e+00 | **PASS** |
  | 4 | output frequency vs N*f_ref = 7.2e+07 Hz | within 1e-3 | 7.20976e+07 Hz (1356 ppm) | **FAIL** |
  | 5 | block LOCK flag, late-window average | >= 0.90 of the rail | 5.470810e-09 V | **FAIL** |
  | 6 | Vctrl inside the usable window | 0.9 .. 2.4 V | 1.483428e+00 .. 1.512957e+00 V | **PASS** |
  | 7 | PFD DN branch asserts (integration guard, not a lock criterion) | mean DN >= 3e-4 of the rail | 5.250876e-02 V | **PASS** |
  ...
  | Vctrl settled, late-window average | 1.496344e+00 V |
  | Vctrl peak-to-peak ripple, late window | 0.02953 V |
  | LOCK flag first asserts at | nan s |
  | PFD UP / DN mean level, late window | 4.097656e-01 V / 5.250876e-02 V |
  | phase error at 14.0u / at 17.0u | 1.957204e-08 s / 1.405254e-08 s |
  | feedback frequency f_fb | 9.01234e+06 Hz |
  | supply current, ref / vco / div domain | -7.857524e-05 / -1.689856e-04 / -8.945827e-05 A |
  | total power at this corner | 1.112 mW |

  **Overall: FAIL** (4 of 7 checks failed).
...
- **Links**:
  ...
  - Netlist snapshot: `sim/pll-top-smoke/netlist-snapshots/20260801-085349-0e5c22d.spice`
  - Raw log: `sim/pll-top-smoke/corners/20260801-085349-0e5c22d/typical_27c_3.30v.log`
  - Acquisition transient: `sim/pll-top-smoke/corners/20260801-085349-0e5c22d/lock_transient_typical_27c_3.30v.csv`
- **Timestamp / author**: 2026-08-01T08:53:51Z, agent-builder (issue #52)
